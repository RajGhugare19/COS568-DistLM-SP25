...
def train(args, train_dataset, model, tokenizer):
    ...
    train_sampler = DistributedSampler(train_dataset) # change - raj
    ...
    for epoch_itr in train_iterator:    # change - raj
            ...
            if args.fp16:
                ...
                for param in model.parameters():
                    ...
                    if args.local_rank == 0:
                        grad_list = [torch.zeros_like(param.grad.data) for _ in range(args.world_size)]
                    else:
                        grad_list = None
        ...
        if iteration_times:
            ...
            logger.info("Epoch {} average time per iteration (excluding first iteration): {:.4f} seconds".format(epoch_itr+1, avg_time))
            with open(os.path.join(args.output_dir, "output.txt"), "a") as writer:
                writer.write(f"Epoch {epoch_itr}: Average iteration time (excluding first iteration): {avg_time}\n")

        else:
            logger.info("Epoch {}: No iteration times recorded.".format(epoch_itr+1))
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        out_file = os.path.join(eval_output_dir, "train_results.txt")
...
def main():
    ...
    parser.add_argument("--master_port", type=str, default=12323)        
